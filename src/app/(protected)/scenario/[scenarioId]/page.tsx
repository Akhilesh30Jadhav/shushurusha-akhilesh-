...
export default function MCQPlayer({ params }: { params: Promise<{ scenarioId: string }> }) {
    ...
    if (!scenario) return <div className="flex min-h-[40vh] items-center justify-center bg-transparent"><Loader2 className="w-12 h-12 animate-spin text-[#FF7A00]" /></div>;
    ...
    return (
        <div className="flex flex-1 min-h-0 flex-col gap-4 md:gap-5 font-sans">

            {/* ══ ROW 1: Full-width Transcript bar ══ */}
            <div className="flex-shrink-0 flex gap-3 sm:gap-4 items-center bg-[#0f172a] rounded-2xl px-3 sm:px-6 py-3 sm:py-4 border border-gray-800 shadow-sm">
                ...
                <div className="flex-1 text-gray-300 text-xs sm:text-sm leading-relaxed font-medium">
            ...
            <div className="flex flex-1 min-h-0 flex-col md:flex-row gap-4 md:gap-5">

                {/* Video — fills full height of this row */}
                <div className="w-full md:w-[45%] flex-shrink-0 self-start aspect-video md:aspect-[16/10] rounded-[1.25rem] md:rounded-[1.75rem] overflow-hidden bg-[#0f172a] relative border-2 md:border-4 border-white shadow-lg">
                ...
                {/* Question + Options */}
                <div className="w-full flex-1 min-h-0 flex flex-col gap-3 md:gap-4 overflow-visible md:overflow-y-auto custom-scrollbar">

                    <div className="flex items-start justify-between gap-3 flex-shrink-0">
                        <h3 className="text-[1.7rem] md:text-2xl font-extrabold text-gray-900 leading-snug">{currentQ.mcq_question}</h3>
                    ...
                    <div className="flex flex-col gap-2.5 md:gap-3 flex-shrink-0">
                        {currentQ.options.map((opt: any) => {
                            ...
                            return (
                                <div key={opt.option_id} onClick={() => handleSelect(opt)} className={`px-3.5 sm:px-5 py-3 sm:py-4 rounded-2xl transition-all duration-200 flex items-center gap-3 sm:gap-4 ${cardClass} ${!hasAnswered ? 'hover:shadow-md hover:-translate-y-0.5 active:scale-[0.99]' : ''}`}>
                                    ...
                                    <div className={`flex-1 text-sm sm:text-base font-bold text-slate-800 leading-snug ${hasAnswered && !isSelected ? 'text-slate-400' : ''}`}>{opt.text}</div>
            ...
            {!hasAnswered && <div className="flex-shrink-0 flex flex-col md:flex-row gap-4 md:gap-5">
